...
func (r *UserAssetCommentPermit) UserAssetId() (*mytype.OID, error) {
	if ok := r.checkFieldPermission("user_asset_id"); !ok {
		return nil, ErrAccessDenied
	}
	return &r.userAssetComment.UserAssetId, nil
}
...
func (r *UserAssetCommentRepo) CountByUserAsset(
	ctx context.Context,
	userAssetId string,
) (int32, error) {
	var n int32
	...
	return data.CountUserAssetCommentByUserAsset(db, userAssetId)
}
...
func (r *UserAssetCommentRepo) GetByUserAsset(
	ctx context.Context,
	userAssetId string,
	po *data.PageOptions,
) ([]*UserAssetCommentPermit, error) {
	...
	userAssetComments, err := data.GetUserAssetCommentByUserAsset(db, userAssetId, po)
	if err != nil {
		return nil, err
	}
	userAssetCommentPermits := make([]*UserAssetCommentPermit, len(userAssetComments))
